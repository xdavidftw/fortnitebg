...
async def on_message(message):
    ...
    if message.content.startswith("/rank"):
        ...
        if ratio >= WOOD_B and ratio <= WOOD_E:
            ...
            await client.add_roles(message.author, role) 
        elif ratio >= HACKEUR_B and ratio <= HACKEUR_E:
            role = discord.utils.get(message.server.roles, name=LISTE[10])
            msg = ("{0.author.mention} Your have been ranked " + role.name).format(message)
            await client.send_message(message.channel, msg)
            msgRatio = "Your ratio: " + str(ratio) + " K/D \n Max level! ¯\\_(ツ)_//¯ " 
            await client.send_message(message.channel, msgRatio)
            await client.send_message(message.channel, print_nextLvl(HACKEUR_B, HACKEUR_E, ratio))
            await client.add_roles(message.author, role) 
        elif ratio == -1:
            msg = "Твоето Discord име не е това което ти е в Fortnite! Изполвай командата ```/nick {Името ти в играта}``` "
            await client.send_message(message.channel, msg)
        elif ratio == -2:
            msg = "Сървърите са офлайн. Опитайте отново по-късно!"
            await client.send_message(message.channel, msg)
